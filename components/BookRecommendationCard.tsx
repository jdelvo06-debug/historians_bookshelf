...
const BookRecommendationCard: React.FC<BookRecommendationCardProps> = ({
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 sm:p-8 animate-fade-in flex flex-col sm:flex-row gap-6 sm:gap-8 relative">
      <div className="absolute top-4 right-4 flex gap-2">
        <button
          onClick={() => onToggleFavorite(recommendation)}
          className="p-2 rounded-full bg-white/50 backdrop-blur-sm text-stone-500 hover:text-red-500 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-all duration-200"
        ...
        {onAddToList && (
          <div className="relative">
            <button
              ...
              className="p-2 rounded-full bg-white/50 backdrop-blur-sm text-stone-500 hover:text-amber-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 transition-all duration-200"
            ...
            {showListMenu && (
              <div className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-stone-200 z-10">
                <div className="p-2">
                  <p className="text-xs font-semibold text-stone-500 uppercase px-2 py-1">Add to list</p>
                  {readingLists.length > 0 ? (
                    ...
                    <p className="text-sm text-stone-500 px-2 py-1">No lists yet</p>
                  )}
                  <div className="border-t border-stone-200 mt-2 pt-2">
                    <div className="flex gap-1">
                      <input
                        ...
                        className="flex-1 px-2 py-1 text-sm border border-stone-300 rounded focus:ring-1 focus:ring-amber-500 focus:border-amber-500"
                        onKeyDown={(e) => e.key === 'Enter' && handleCreateAndAdd()}
                      />
                      <button
                        onClick={handleCreateAndAdd}
                        disabled={!newListName.trim()}
                        className="px-2 py-1 text-sm bg-amber-800 text-white rounded hover:bg-amber-900 disabled:bg-stone-300 disabled:cursor-not-allowed"
                      >
                        Add
                      </button>
                    </div>
      ...
      <div className="flex flex-col flex-grow">
        <div className="flex-grow">
          <h2 className="text-2xl sm:text-3xl font-bold text-amber-900 font-serif pr-10">{recommendation.title}</h2>
          <p className="mt-2 text-lg text-stone-600 italic">by {recommendation.author}</p>
          <div className="w-24 h-1 bg-amber-200 my-6 rounded"></div>
        ...
        <div className="mt-6 text-right">
          <a
            ...
            className="inline-flex items-center px-4 py-2 bg-amber-800 text-white font-semibold rounded-lg hover:bg-amber-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-700 transition-colors duration-200 text-sm"
