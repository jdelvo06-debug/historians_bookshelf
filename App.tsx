...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-stone-50 text-stone-800 flex flex-col items-center p-4 sm:p-6 lg:p-8">
      <main className="w-full max-w-2xl mx-auto flex flex-col items-center">
        <header className="text-center my-8">
          <h1 className="text-4xl sm:text-5xl font-bold text-amber-900 font-serif">Historian's Bookshelf</h1>
          <p className="mt-2 text-lg text-stone-600">Your personal AI librarian for historical literature.</p>
        </header>
        ...
        <div className="w-full my-6">
          <div className="flex border-b border-stone-200">
            <button
              ...
              className={`px-4 py-2 text-lg font-semibold transition-colors duration-200 ${view === 'search' ? 'border-b-2 border-amber-800 text-amber-800' : 'text-stone-500 hover:text-amber-700'}`}
            ...
            <button
              ...
              className={`px-4 py-2 text-lg font-semibold transition-colors duration-200 flex items-center gap-2 ${view === 'favorites' ? 'border-b-2 border-amber-800 text-amber-800' : 'text-stone-500 hover:text-amber-700'}`}
              aria-current={view === 'favorites'}
            >
              Favorites <span className="bg-amber-200 text-amber-800 text-xs font-bold px-2 py-0.5 rounded-full">{favorites.length}</span>
            </button>
            <button
              onClick={() => setView('lists')}
              className={`px-4 py-2 text-lg font-semibold transition-colors duration-200 flex items-center gap-2 ${view === 'lists' ? 'border-b-2 border-amber-800 text-amber-800' : 'text-stone-500 hover:text-amber-700'}`}
              aria-current={view === 'lists'}
            >
              Lists <span className="bg-amber-200 text-amber-800 text-xs font-bold px-2 py-0.5 rounded-full">{readingLists.length}</span>
            </button>
          </div>
        </div>
            ...
            <>
              ...
              {recommendations && recommendations.length === 0 && (
                 <div className="text-center p-6 bg-stone-100/50 rounded-lg">
                    <h3 className="text-xl font-semibold text-stone-700 font-serif">No Results</h3>
                    <p className="mt-2 text-stone-600">We couldn't find any book recommendations for that topic. Please try a different search term.</p>
                </div>
              )}
          ...
          {view === 'favorites' && (
            <div className="space-y-8">
              {favorites.length > 0 ? (
                ...
                <div className="text-center p-6 bg-stone-100/50 rounded-lg">
                  <h3 className="text-xl font-semibold text-stone-700 font-serif">No Saved Books</h3>
                  <p className="mt-2 text-stone-600">You haven't added any books to your favorites yet. Use the heart icon on a recommendation to save it here.</p>
                </div>
              )}
            </div>
          )}
              ...
              {!selectedListId ? (
                <>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-amber-800 text-white font-semibold rounded-lg hover:bg-amber-900 transition-colors"
                    >
                      Create List
                    </button>
                  </div>
                  {readingLists.length > 0 ? (
                    <div className="grid gap-4">
                      {readingLists.map(list => (
                        ...
                        >
                          ...
                          >
                            <h3 className="text-lg font-semibold text-amber-900 font-serif">{list.name}</h3>
                            <p className="text-sm text-stone-500">{list.books.length} book{list.books.length !== 1 ? 's' : ''}</p>
                          </button>
                          <button
                    ...
                    <div className="text-center p-6 bg-stone-100/50 rounded-lg">
                      <h3 className="text-xl font-semibold text-stone-700 font-serif">No Reading Lists</h3>
                      <p className="mt-2 text-stone-600">Create a reading list to organize your book recommendations by theme or project.</p>
                    </div>
                  )}
                </>
              ) : (
                <>
                  <button
                    ...
                    className="flex items-center gap-2 text-amber-800 hover:text-amber-900 font-semibold mb-4"
                  >
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        <h2 className="text-2xl font-bold text-amber-900 font-serif mb-4">{selectedList.name}</h2>
                        {selectedList.books.length > 0 ? (
                            ...
                            {selectedList.books.map((book, index) => (
                              <div key={index} className="relative">
                                ...
                                <button
                                  ...
                                  className="absolute top-4 left-4 p-2 rounded-full bg-white/50 backdrop-blur-sm text-stone-500 hover:text-red-500 transition-colors"
                          ...
                          <div className="text-center p-6 bg-stone-100/50 rounded-lg">
                            <h3 className="text-xl font-semibold text-stone-700 font-serif">Empty List</h3>
                            <p className="mt-2 text-stone-600">Search for books and use the + button to add them to this list.</p>
                          </div>
                        )}
                      </>
                    );
